...
// NMH***** VOLTAGE SENSOR BOARD
// const int voltageSensorPin = A2;

// Voltage detection Voltage Divider board
const int   VBATT_IN_PIN        = A7;  // Analog input pin for battery voltage
const int   VBATT1_IN_PIN       = A6;  // Analog input pin for the lower battery voltage (battery between ground and 12V)
//
...
// ** Change to 27.5 if cannot ZERO
  ...
  lcd.setCursor(0, 0);
  lcd.print("Battery:   ");
  lcd.print(batteryVoltageTotal);     // batteryVoltage - original
  lcd.print("%");
  lcd.setCursor(0, 1);
  if (motorMPH > 2) {
      lcd.print("Speed:       MPH");
  ...
  sendData('V', batteryVoltageTotal);
  sendData('W', wheelRPM);
  sendData('R', motorRPM);
...
int throttleValue = analogRead(throttlePin);
  ...
  String my_String = "{'1':'"+String(motorTemperature)+"','2':'"+String(batteryTemperature)+"','3':'"+String(motorRPM)+"','4':'"+String(wheelRPM)+"','5':'"+String(batteryVoltageTotal)+"','6':'"+String(current)+"'}";
...
// Original Voltage Read Function
//float getVoltage() {
...
// Voltage divider board function
